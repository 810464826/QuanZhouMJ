 ...
 *
 */
public class ThreeYouZhongService extends OutCardService {
	...
	public void doService(Session session, ReceiveMsg action) {
		...
		// 设置当前是单游中
		playerInfo.addHuInfo(MJConst.YOU_JINGING, 3);
		int youjin = room.getArg(MJConst.YOU_JINGING);
		room.addArg(MJConst.YOU_JINGING, youjin > 3 ? youjin : 3);
		String card = dealOutCardInfo(action, playerInfo, room);
		System.err.println(
				"ThreeYouZhong ting: " + playerInfo.getUserInfo().getUserId() + ":::" + playerInfo.getTingCards());
