 ...
 *
 */
public class QZMJRoom extends Room {

	private static final int FEN2 = 200;

	private static final int FEN1 = 100;

	private static Log log = Log.getLog(QZMJRoom.class);
	...
	public synchronized void readyEvent() {
		...
		List<CheckHuaMsg> huaList = MJCheckKit.iteratorRoomUsers(this, new RoomUserCallable<CheckHuaMsg>() {
			...
			public CheckHuaMsg call(Room room, Entry<Integer, Long> user) throws Exception {
				...
				// 保存手牌
				playerInfo.setHandCards(Collections.synchronizedList(new ArrayList<String>(Arrays.asList(cards))));
	...
	public void joinEvent(PlayerInfo playerInfo) {
		...
		playerInfo.getUserInfo().setReady(false);
	...
	public void creatEvent(PlayerInfo playerInfo) {
		this.baseFen = (int) Math.pow(2, this.opt[0]);
	...
	public void juScore(PlayerInfo huPlayer, OptCheckMsg huMsg) {
		...
		// 非分数打法,连庄底分翻翻
		if (this.totalJuNum != QZMJRule.F_ONE_HUNDRED && this.totalJuNum != QZMJRule.F_TWO_HUNDRED) {
		...
		clearCurrentScore();
		// 计算本局胡牌得分
		calcCurrentScore(huId, 0, baseHuScore * 3, -baseHuScore);
		...
		while (it.hasNext()) {
			...
			if (jinCount > 0) {
				if (huId == userId) {
					...
					// 没胡人的金,没胡的人给（2个人）
					calcJinScore(userId, huId, jinCount * this.baseFen * 2, -jinCount * this.baseFen);
				}
			}
	...
	private void calcJinScore(long userId, long huId, int selfScore, int otherScore) {
		Iterator<Entry<Integer, Long>> it = this.getRoomUsers().entrySet().iterator();
		while (it.hasNext()) {
			Entry<Integer, Long> user = it.next();
			long myUserId = user.getValue();
